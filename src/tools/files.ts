...
export const readFileToolDef: Anthropic.Tool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      lines: { type: "number", description: "Max lines to return (optional, defaults to all)" },
    },
    required: ["path"],
  },
};
...
export async function readFile(input: { path: string; lines?: number }): Promise<string> {
...
export async function writeFile(input: { path: string; content: string; append?: boolean }): Promise<string> {
  const resolved = path.resolve(input.path);
