...
export const MEMORY_FILE = path.join(os.homedir(), '.opensage', 'memory.json')
  ...
  input_schema: {
    ...
    properties: {
      content: {
        type: 'string',
        description:
          'The fact or note to remember. Be concise and specific (one sentence max).',
      },
      category: {
        type: 'string',
        description:
...
export async function saveMemoryTool(input: {
  content: string
  category?: string
}): Promise<string> {
