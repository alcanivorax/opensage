...
export async function getSystemInfo(input: { type?: string }): Promise<string> {
  ...
  if (want === 'network') {
    ...
    for (const [name, addrs] of Object.entries(ifaces)) {
      for (const addr of addrs ?? []) {
        ...
        lines.push(
          `${name.padEnd(12)} ${addr.family.padEnd(6)} ${addr.address}`
        )
      }
    }
...
export async function writeClipboard(input: { text: string }): Promise<string> {
  const { text } = input
  const p = process.platform

  // Write to a temp file to avoid shell escaping issues with arbitrary content
  const tmp = os.tmpdir() + '/opensage_clip_' + Date.now() + '.txt'
