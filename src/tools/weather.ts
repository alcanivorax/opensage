  ...
  input_schema: {
    ...
    properties: {
      ...
      forecast: {
        type: 'boolean',
        description:
          'Include the 3-day hourly forecast. Defaults to true.',
      },
    },
    required: [],
  },
...
export async function getWeather(input: {
  ...
  try {
    const res = await fetch(url, {
      headers: { 'User-Agent': 'aichat/4.1 personal-assistant' },
  ...
  const wind      = metric ? `${cur.windspeedKmph} km/h` : `${Math.round(parseInt(cur.windspeedKmph) * 0.621)} mph`
  ...
  lines.push(`Wind         : ${wind} ${cur.winddir16Point}  (${windDesc(windSpd)})`)
  ...
  if (showForecast && days.length > 0) {
    ...
    for (const day of days) {
      ...
      lines.push(``)
      lines.push(`${dateStr.padEnd(16)}  ${lo} → ${hi}  avg ${avg}  UV ${day.uvIndex}`)
