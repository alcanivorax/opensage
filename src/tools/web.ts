...
export async function webFetch(input: { url: string }): Promise<string> {
  try {
    const res = await fetch(input.url, {
      headers: { "User-Agent": "aichat/2.1 terminal assistant" },
...
export async function webSearch(input: { query: string }): Promise<string> {
  ...
  try {
    ...
    const res = await fetch(
      ...
      { headers: { "User-Agent": "aichat/2.1" }, signal: AbortSignal.timeout(8_000) }
    );
  ...
  if (results.length === 0) {
    try {
      ...
      const urls = [...html.matchAll(/class="result__url"[^>]*>([\s\S]*?)<\/a>/g)]
        .slice(0, 5)
        .map((m) => m[1].replace(/<[^>]+>/g, "").trim());
